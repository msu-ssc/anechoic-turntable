...
def append_changelog_entry(changelog: Path, version: str, release_date: str, note: str) -> None:
    """Append one controller release entry to the changelog."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    bump_parser = subparsers.add_parser("bump", help="bump and synchronize release versions")
    bump_parser.add_argument("bump_type", choices=("major", "minor", "patch"))
    bump_parser.add_argument("--release-date", default=datetime.now(timezone.utc).date().isoformat())
    bump_parser.add_argument("--release-note-file", type=Path)
    return parser
...
def main() -> None:
    ...
    version = synchronize_versions(repository_root, arguments.bump_type)
    if arguments.release_note_file is not None:
        note = arguments.release_note_file.read_text(encoding="utf-8")
        append_changelog_entry(repository_root / CHANGELOG, version, arguments.release_date, note)
    print(version)
